...
public class ColorInfo {
    ...
    public ArrayList<ListViewItem> getColorList(String company,String type,String colorName,boolean iHave){
        ...
        if(null != company && !"".equals(company)){
            paramKey += " AND " + COL_COMPANY + " LIKE '%" + company + "%'";
        }
        if(null != type && !"All".equals(type) && !"".equals(type)){
        ...
        if(count > 0){
            ...
            for(int i=0; i < count; i++){
                ...
                if( null != item.getEtStock() && Integer.parseInt(item.getEtStock()) > 0 ){
                    item.setFlipType(1);
                }else{
                    item.setFlipType(0);
                }
